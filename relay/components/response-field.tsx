...
export default function ResponseField({incidents,selected,onSelect,ambientPaused=false}:{incidents:Incident[];selected:string;onSelect:(id:string)=>void;ambientPaused?:boolean}){
 ...
 selection.current=selected;motion.current=!(paused||ambientPaused);
 useEffect(()=>{invalidate.current();},[selected,paused,ambientPaused]);
 useEffect(()=>{
  ...
  import('three').then(T=>{
   ...
   for(let a=0;a<180;a++)for(let b=0;b<132;b++){const x=(a/179-.5)*29,z=(b/131-.5)*21,h=elevation(x,z),edge=Math.max(Math.abs(x)/14.5,Math.abs(z)/10.5);positions.push(x,h,z);const hot=Math.min(h/5,1),c=new T.Color().lerpColors(new T.Color('#61787e'),new T.Color('#eaaa70'),hot);const fade=Math.max(.15,1-edge**5);colors.push(c.r*fade,c.g*fade,c.b*fade);}
   ...
   const linesGeometry=new T.BufferGeometry();linesGeometry.setAttribute('position',new T.Float32BufferAttribute(gridPositions,3));world.add(new T.LineSegments(linesGeometry,new T.LineBasicMaterial({color:'#97adb3',transparent:true,opacity:.12})));
   const grid=new T.GridHelper(44,44,0x233748,0x172532);grid.position.y=-.6;const gm=grid.material as Three.Material;gm.transparent=true;gm.opacity=.45;world.add(grid);
   ...
   for(const p of signalPoints){
    const y=elevation(p.x,p.z),color=p.incident.status==='dispatched'?0x9be4ef:p.incident.severity==='critical'?0xff8452:0xe8c897;
