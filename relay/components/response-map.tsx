...
export default function ResponseMap({incidents,teams,selected,onSelect,route}:{incidents:Incident[];teams:Team[];selected:string;onSelect:(id:string)=>void;route?:{team:Team;incident:Incident}}){
 ...
 {showTeams&&teams.map(team=>{const p=point(team.lat,team.lng);return <g key={team.id} transform={`translate(${p.x},${p.y})`}><circle r="14" fill="#243d37" stroke="#7eb69c" strokeWidth="1.5"/><path d="M-5 0h10M0-5v10" stroke="#ace4c7" strokeWidth="2"/><title>{team.name}</title></g>;})}
 ...
 <circle r={active?18:13} fill={i.status==='dispatched'?'#61ad91':i.severity==='critical'?'#ff754c':i.severity==='high'?'#eeb45c':'#a9bcc3'} stroke="#1a2425" strokeWidth="3"/>
